...
public:
    ...
    [[nodiscard]] size_t in_degree(const T& node) const;
    vector<pair<const T*, size_t>> out_degrees(bool sorted = false, bool descent = false) const;
    // vector<pair<const T*, size_t>> in_degrees() const;
...
private:
    ...
    unordered_map<T, size_t> in_degree_map;
};
// Complejidad 0(1)
...
template <class T>
size_t ALDWGraph<T>::in_degree(const T& node) const {
    if (!node_ptr_map.count(node)) return 0;
    auto it = in_degree_map.find(node);
    return it != in_degree_map.end() ? it->second : 0;
}
...
bool ALDWGraph<T>::remove_node(const T& node)
{
    ...
    in_degree_map.erase(node); // Elimina entrada del nodo borrado
    adj_list.erase(node);
...
{ // Se hace la verificacion si ambos nodos existen en el grafo
    ...
    if (vec.size() < original_size) // Si el tamaño del vector cambia o vario esto significa que se borro una arista
    {
        ...
        --in_degree_map[to];
        if (in_degree_map[to] == 0) {
            in_degree_map.erase(to); // Limpieza opcional
        }
