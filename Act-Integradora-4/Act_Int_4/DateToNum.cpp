...
DateToNum::DateToNum(const DateToNum& other)
{
    time_string = other.time_string;
    timeStruct = other.timeStruct;
    for (int i = 0; i < numOfElements; i++)
    {
        int_data[i] = other.int_data[i];
    }
}
...
int DateToNum::days_to_timestamp_diff(const int& days) // Funcion que convierte un numero a dias y ademas calcula
{
    constexpr int day_1_timestamp = 1735711200; // valor del timestamp constante
    constexpr int year = 2025; // se define el año para construir la estrutura del tiempo
    std::tm timeStruct{}; // crea la estructura en tm en cero, para la fecha
    timeStruct.tm_year = year - 1900; // los años empezaran a contar desde el año 1900
    timeStruct.tm_mday = days + 1; // Se inicia la suma porque en tm empiezan desde 1 los dias
    timeStruct.tm_mon = 0; // Los meses van desde 0 a 11 porque 0 es enero
    timeStruct.tm_isdst = 0; // Desactiva el horario de verano

    const std::time_t time_stamp = std::mktime(&timeStruct); // Convierte la estructura tm en un timestamp de Unix

    return static_cast<int>(time_stamp) - day_1_timestamp;  // valor representa los segundos transcurridos desde ese día base

}

DateToNum& DateToNum::operator=(const DateToNum& other)
{
    if (this != &other)
    {
        time_string = other.time_string;
