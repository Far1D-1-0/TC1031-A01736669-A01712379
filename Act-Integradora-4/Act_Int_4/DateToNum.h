...
public:
    ...
    DateToNum(const DateToNum& other); // Constructor de copia
    ...
    DateToNum& operator=(const DateToNum& other); // Operador de asignacion

    ~DateToNum(); //Destructor de la clase
private:
    static constexpr int numOfElements = 8; // Total de elementos numericos
    std::tm timeStruct{}; // Almacena los datos desglosados
    string time_string;
    string format = "%Y-%m-%d %H:%M:%S"; //Define el formato
    int int_data[numOfElements]{}; // Almacena distintos elementos numericos
