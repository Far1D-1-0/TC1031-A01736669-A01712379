...
void fileReader::setline(const size_t& line)
{
    ...
    infile.clear(); //Limpia los fñags de error esto antes de poscionarse en la linea determinada
...
void fileReader::indexLines()
{
    ...
    while (true)
    {
        ...
        if (string line; !std::getline(infile, line)) { //Intenta leer una linea
    ...
    lines = positions.size(); //Por ultimo guarda la cantidad total de lineas oindexadas
}
