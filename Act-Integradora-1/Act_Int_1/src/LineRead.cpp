...
void LineRead::readLine(string line)
{
    ...
    for (int i = 0; i < numOfElements; i++) {
        ...
        elements.push_back(line.substr(0, line.find(separator))); //Extrae el primer elemneto que esta antes del separador
        line = line.substr(line.find(separator) + 1); //Borra el pirmer elemento y luego el seprador de la linea
        if (line.empty()) {
            break;
        }
    }
